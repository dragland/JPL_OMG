...
public class updateHUD : MonoBehaviour {
	...
	public float ELEVATION_SCALE_CONSTANT = 14.407842f;
	public float ELEVATION_OFFSET_CONSTANT = -1513f;
	/* ****************  GLOBAL OBJECTS  ****************  */
	...
	*/
	void updatePosHUD(){
		string myX = ((Camera.main.transform.position.x) * -1).ToString("0.00");
		string myY = (Camera.main.transform.position.z).ToString("0.00");
		string myZ = ((Camera.main.transform.position.y)).ToString("0.00");
		text_2.text = "X = " + myX + "\nY = " + myY + "\nZ = " + myZ;
	}
	...
	*/
	void updateDataHUD(){
		...
		string myX = ((hit.point.x) * -1).ToString ("0.00");
		string myY = (hit.point.z).ToString ("0.00");
		string myZ = ((hit.point.y)).ToString ("0.00");
		string range = (hit.distance).ToString ("0.00");
		string RGBA = getRGBA(hit.textureCoord.x, hit.textureCoord.y);
	...
	*/
	string getElevation(float textureX, float textureY){
		...
		elevation *= ELEVATION_SCALE_CONSTANT;
		elevation += ELEVATION_OFFSET_CONSTANT;
